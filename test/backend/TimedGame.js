 ...
 */
describe("backend/TimedGame", function() {
  ...
  it('timeout and end game', () => {
    ...
    const game = new BackendGame({
      ...
      timerType: BackendGame.Timer.TURN,
      timeAllowed: 1 / 60
    });
    let nextTurn = 0;
    ...
    const handle = (turn, event, seqNo) => {
      ...
      case 2:
        ...
        assert.equal(turn.type, BackendGame.Turns.TIMED_OUT);
      ...
      case 9:
        ...
        assert.equal(turn.endState, BackendGame.State.TWO_PASSES);
        assert.equal(turn.gameKey, game.key);
    ...
    socket.on(BackendGame.Notify.TICK, () => {});
  ...
  it('pass and overtime', () => {
    ...
    const game = new BackendGame({
      ...
      //_debug: console.debug,
      noPlayerShuffle: true,
      timerType: BackendGame.Timer.GAME,
      timeAllowed: 1 / 60,
      timePenalty: 60
    });
    let nextTurn = 0;
    ...
    .then(() => game.pass(human2, BackendGame.Turns.PASSED))
    .then(() => awaitTick())
    .then(() => game.pass(human1, BackendGame.Turns.PASSED))
    .then(() => game.pass(human2, BackendGame.Turns.PASSED))
    .then(() => socket.wait());
