...
import { TestSocket } from '../../test/TestSocket.js';
...
describe("backend/RobotPlays", () => {
  ...
  it("robot to play when ready", () => {
    ...
    .then(game => {
      ...
      // robot.
      game.state = BackendGame.State.PLAYING;
  ...
  it("robot cannot play first", () => {
    ...
    const robot = new Player(
      {name:"Machine", key:"robot",
       isRobot: true, canChallenge: true}, BackendGame.CLASSES);
    const human = new Player(
      {name:"Man", key:"human", isRobot: false}, BackendGame.CLASSES);
    const socket = new TestSocket();
    socket.on(BackendGame.Notify.CONNECTIONS, () => {});
    socket.on(BackendGame.Notify.TURN, (turn, event, seqNo) => {
      switch (seqNo) {
      case 1:
        assert.equal(turn.type, BackendGame.Turns.PASSED);
  ...
  it("robot can play first", () => {
    ...
    const robot = new Player({
      name: "Robot 1", key:"robot", isRobot: true}, BackendGame.CLASSES);
    const human = new Player({
      name: "Human 2", key:"human", isRobot: false}, BackendGame.CLASSES);

    const socket = new TestSocket();
    socket.on(BackendGame.Notify.TURN, (data, event, seqNo) => {
      assert.equal(seqNo, 1);
      assert.equal(event, BackendGame.Notify.TURN);
      assert(data.type, BackendGame.Turns.PLAYED);
    ...
    .on(BackendGame.Notify.CONNECTIONS, () => {})
    .on("*", (data, event) => {
      console.error("UNEXPECTED EVENT", event);
      assert.fail(event);
    });
  ...
  it("robot play second", () => {
    ...
    let human = new Player({
      name: "Human 1", key:"human", isRobot: false}, BackendGame.CLASSES);
    let robot = new Player({
      name: "Robot 2", key:"robot", isRobot: true}, BackendGame.CLASSES);
    ...
    const handle = (data, event, seqNo) => {
      assert.equal(event, BackendGame.Notify.TURN);
      assert.equal(data.type, BackendGame.Turns.PLAYED);
    ...
    socket.on(BackendGame.Notify.TURN, handle);
    socket.on(BackendGame.Notify.CONNECTIONS, () => {});
  ...
  it("robot confirm game over", () => {
    ...
    const handle = (turn, event, seqNo) => {
      switch (seqNo) {
      case 1:
        assert.equal(turn.type, BackendGame.Turns.PLAYED);
        break;
      case 2:
        assert.equal(event, BackendGame.Notify.TURN);
        assert.equal(turn.type, BackendGame.Turns.GAME_ENDED);
        assert.equal(turn.endState, BackendGame.State.GAME_OVER);
        const exp = {};
    ...
    // robot to confirmBackendGameOver
    .then(() => socket.wait());
  });
  ...
  it("robot challenge and make last play", () => {
    ...
    socket.on(BackendGame.Notify.TURN, (turn, event, seqNo) => {
      ...
      case 1:
        ...
        break;
      case 2:
        assert.equal(turn.type, BackendGame.Turns.CHALLENGE_WON);
