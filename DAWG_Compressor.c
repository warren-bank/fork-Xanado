...
// 1) "Lexicon.txt" is a word list, where all characters are capital, and the number of words is written on the very first line. Linux format, so no DOS CR character. No blank lines.
...
// To use the DAWG you must understand the procedures for extracting information from the unsigned integer nodes. Sample code
// that does this can be found in js/scrabble/Dictionary.js
...
void MakeMeAllCapital(char *RawWord){
	...
	for ( X = 0; X < strlen(RawWord); X++ ){
		if (RawWord[X] >= 'a' && RawWord[X] <= 'z' )
		  RawWord[X] = RawWord[X] + BIG_IT_UP;
	}
}
...
void TnodeInsertParaNode(TnodePtr AboveTnode, char ThisLetter, char WordEnder, int StartDepth){
	...
	if ( TNODE_CHILD(AboveTnode) == NULL )
	  TNODE_SET_CHILD(AboveTnode, TnodeInit(ThisLetter, NULL, WordEnder, TNODE_LEVEL(AboveTnode) + 1, StartDepth, AboveTnode, TRUE));
	...
	// Case 3: The ParaList exists and ThisLetter is not first in the list.  This is the default case condition:  "if ( TNODE_LETTER(TNODE_CHILD(AboveTnode)) < ThisLetter )".
		...
		if ( Holder != NULL )
		  TNODE_SET_PARENTAL_UNIT(Holder, TNODE_NEXT(Currently));
	}
}
...
unsigned int *DawgInit(char **Dictionary, int *SegmentLenghts, int MaxStringLength, int MinStringLength){
	...
	for ( X = 0; X < MAX; X++ )
	  HolderOfAllTnodePointers[X] = (TnodePtr *)malloc(NodeNumberCounterInit[X]*sizeof(TnodePtr));
	...
	// Flag all of the redundant nodes.
	...
	printf("\nStep 8 - Begin to tag redundant nodes as dangled - Use recursion because only direct children are considered for elimination to keep the remaining lists intact.\n");
	...
	for ( Y = (MaxStringLength - 1); Y >= 0 ; Y--) {
		...
		for ( W = 0; W < (NodeNumberCounterInit[Y] - 1); W++ ) {
			...
			for ( X = W + 1; X < NodeNumberCounterInit[Y]; X++ ) {
				if ( TNODE_DANGLING(HolderOfAllTnodePointers[Y][X]) == FALSE
					 && TNODE_DIRECT_CHILD(HolderOfAllTnodePointers[Y][X]) == TRUE ) {
	...
	if ( TotalDangled == TotalDangledCheck )
	  printf("The total number of nodes dangled adds up.\n");
	else
	  printf("Something went terribly wrong, so fix it.\n");
	...
	if ( NumberOfLivingNodes == IndexCount )
	  printf("The numbers add up properly once again.\n");
	else {
	...
	for ( X = 0; X < MaxStringLength; X++ )
	  for ( W = 0; W < NodeNumberCounterInit[X]; W++ )
		free(HolderOfAllTnodePointers[X][W]);
	free(TemporaryTrie);
	free(NodeNumberCounter);
	free(NodeNumberCounterInit);
	for ( X = 0; X < MaxStringLength; X++ )
	  if (HolderOfAllTnodePointers[X]!= NULL)
...
int main() {
	...
	for ( X = 0; X < FirstLineIsSize; X++ ) {
		...
		if ( CurrentLength <= MAX )
		  strcpy(&((AllWordsInEnglish[CurrentLength])[(CurrentTracker[CurrentLength]*(CurrentLength + 1))]), LexiconInRam[X]);
	...
	for ( X = 0; X <= MAX; X++ )
	  DictionarySizeIndex[X] = KeepTracker[X];
